...
# V. GINGER'S ADVICE, ANALYSES
## 1. Contract & Reservation Cross-tabs:
  Columns: herdMgmt_thisYr_wintPast/herdMgmt_lastYr_wintPast/herdMgmt_thisYr_springPast/
  herdMgmt_lastYr_springPast/herdMgmt_thisYr_DzudPast/herdMgmt_lastYr_DzudPast/wintContract/
  wintPasContract/sprCampContract/sprPasContract
```{r sidewaystable_demo, results='asis', echo=FALSE}
merged_data <- base_HERDMGMT %>%
  inner_join(base_TENURE, by = "Ref")

# Shortened label mappings
res_labels <- c(
  "herdMgmt_thisYr_wintPast" = "WintThisYr",
  "herdMgmt_lastYr_wintPast" = "WintLastYr",
  "herdMgmt_thisYr_springPast" = "SprThisYr",
  "herdMgmt_lastYr_springPast" = "SprLastYr",
  "herdMgmt_thisYr_DzudPast" = "DzudThisYr",
  "herdMgmt_lastYr_DzudPast" = "DzudLastYr"
)

con_labels <- c(
  "wintContract" = "WintCon",
  "wintPasContract" = "WintPasCon",
  "sprCampContract" = "SprCampCon",
  "sprPasContract" = "SprPasCon"
)

contract_cols <- names(con_labels)
reservation_cols <- names(res_labels)
...
cross_table <- cross_counts %>%
  mutate(
    res_label = paste0(res_labels[res_col], ": ", res_val),
    con_label = paste0(con_labels[con_col], ": ", con_val)
  ) %>%
  select(res_label, con_label, count)

# Make sure we order by shortened column labels
ordered_con_labels <- unlist(lapply(contract_cols, function(col) {
  c(paste0(con_labels[col], ": No"), paste0(con_labels[col], ": Yes"))
}))
...
# Display
knitr::kable(cross_table_wide, format = "latex", booktabs = TRUE) %>%
kableExtra::kable_styling(latex_options = "hold_position", font_size = 4)

```


## 2. Contract Percentages:
  Columns: wintCamp/wintContract/wintPas/wintPasContract/sprCamp/sprCampContract/sprPasContract
```{r, echo=FALSE}
cols_to_summarize <- c(
...
## 3. Reservation Percentages:
  Columns: herdMgmt_thisYr_wintPast/herdMgmt_lastYr_wintPast/herdMgmt_thisYr_springPast/
  herdMgmt_lastYr_springPast/herdMgmt_thisYr_DzudPast/herdMgmt_lastYr_DzudPast
```{r, echo=FALSE}
cols_to_summarize <- c(
...
## 5. Reservation Bar Chart:
  Columns: herdMgmt_thisYr_wintPast/herdMgmt_lastYr_wintPast/herdMgmt_thisYr_springPast/
  herdMgmt_lastYr_springPast/herdMgmt_thisYr_DzudPast/herdMgmt_lastYr_DzudPast
```{r, plot_yesno, echo=FALSE, fig.width=7, fig.height=4.5, fig.align='center', out.width='\\linewidth', dev='cairo_pdf'}
cols_to_plot2 <- c(
...
ggplot(plot_data2, aes(x = variable, y = percent, fill = response)) +
  ...
  coord_flip() +
  scale_fill_manual(values = c("Yes" = "#1b9e77", "No" = "#d95f02")) +
  # move the (now) categorical axis labels to the right
...
## 6. Herding Distance Histograms:
  Columns: herdMgmt_sumDailyDist/herdMgmt_wintDailyDist
```{r, echo=FALSE}
...
## 7. Means of Herding Travel, plot:
  Columns: herdMgmt_10yrsAgo_herdTravel/herdMgmt_5YrsAgo_herdTravel/herdMgmt_lastYr_herdTravel
```{r, echo=FALSE}
travel_long_clean <- base_HERDMGMT %>%
  select(Soum,
...
## 8. Distance of Herding Travel, plot:
  Columns: herdMgmt_10yrsAgo_dailyDist/herdMgmt_5YrsAgo_dailyDist/herdMgmt_lastYr_dailyDist
```{r, echo=FALSE}
